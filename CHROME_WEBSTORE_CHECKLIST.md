...
```bash
# Build the shippable bundle first, then package dist/ (never the
# source directories: offscreen.js must be bundled).
cd open-capture-extension
npm ci
npm run build
cd dist && zip -r "../snapcap-extension-v$(node -p "require('../manifest.json').version").zip" . -x "*.DS_Store" "*.zip"
```
...
1. Click "New Item"
2. Upload `snapcap-extension-v<version>.zip`
...
| Firebase (Blaze plan, optional backend only) | Pay-as-you-go |
