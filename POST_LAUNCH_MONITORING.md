...
| Weekly Retention         | Growing | < 10% week-over-week drop |
| Avg Capture Time         | < 2s    | > 5s                      |
| Avg Recording Start Time | < 1s    | > 3s                      |

### Adoption Metrics

(SnapCap is free for everyone — there are no revenue or conversion metrics.)

- Installs / uninstalls
- Weekly active users
- Feature usage mix (screenshot vs. recording vs. editor)
- Optional self-hosted backend adoption
...
| Usage Summary       | Daily captures/recordings | Email   |
...
3. Check external dependencies (Firebase)
...
### Sync / Upload Failures (optional self-hosted backend)

1. Check Firebase Auth + Firestore status
2. Check API error rate in Sentry
3. Check storage quota and rules
4. Verify the extension falls back to local-only mode

### High Uninstall Rate

1. Read recent Chrome Web Store reviews
2. Check recent releases for regressions
3. Survey users via the feedback channel
4. Prioritise the top reported bugs
